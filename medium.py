...
def get_profile_urls(pub_url):
    ...
    no_of_pagedowns = 1  # 100 For complete scroll (JTC)
    ...
    # Get tag
    a_tag = browser.find_elements_by_class_name(
        "link.link--primary.u-accentColor--hoverTextNormal")
...
def get_writer_profile(browser, writer_profile_df, writer_profiles_col):
    ...
    try:
        ...
        create_new_entry = pd.DataFrame(
            writer_profile, columns=writer_profiles_col)

        # Appends new entry to posts_df
        writer_profile_df = writer_profile_df.append(
...
def get_posts(browser, posts_df, post_details_col):
    ...
    for row in page_content.find_all('div', class_=ends_with_yc):
        ...
        try:
            ...
            publisher.append(' '.join(pub_tag.text.split()[
                             pub_tag.text.split().index("in")+1:]))
            user_name.append(' '.join(pub_tag.text.split()[
                             :pub_tag.text.split().index("in")]))
        except:
        ...
        try:
            # Create new entry
            create_new_entry = pd.DataFrame(
                post_details, columns=post_details_col)
...
def extract_information(url, posts_df, writer_profile_df, post_details_col, writer_profiles_col):
    ...
    # Get profiles
    writer_profile_df, error_count_profiles = get_writer_profile(
        browser, writer_profile_df, writer_profiles_col)
...
# Get profile urls
_, url_list_original = get_profile_urls(
    "https://medium.com/search/users?q=towards%20data%20science")

url_list = url_list_original[-5:]  # For rapid testing (JTC)

# Set column names
writer_profiles_col = ["user_name", "user_profile_desc",
                       "user_followers", "top_writer_flag"]
post_details_col = ["user_name", "title",
                    "publisher", "claps", "date_posted", "read_time"]
...
for url in url_list:
    ...
    posts_df, writer_profile_df, error_retrieved = extract_information(
        url, posts_df, writer_profile_df, post_details_col, writer_profiles_col)
    ...
    if save_state % 50 == 1:
        ...
        writer_profile_df.to_csv(r'tds_scrape/writer_profile_df.csv')
        posts_df.to_csv(r'tds_scrape/posts_df.csv')

    save_state += 1

    print("Processed: %s / %s -- Elapse Time: %s" %
          (time_counter, len(url_list), datetime.now()-t0))
    print(f"Errors due to input mismatch: {error_count}")

# Write to CSVs

# writer_profile_df.to_csv(r'C:\Users\writer_profile_df.csv')
# posts_df.to_csv(r'C:\Users\posts_df.csv   /usr/local/bin/chromedriver')


# writer_profile_df.to_csv(r'/Users/jotorcoswriter_profile_df.csv')
# posts_df.to_csv(r'/Users/jotorcos/posts_df.csv')

# directory_of_python_script = os.path.dirname(os.path.abspath(__file__))
# writer_profile_df.to_csv(os.path.join(
#     directory_of_python_script, "writer_profile_df.csv"))
# posts_df.to_csv(os.path.join(directory_of_python_script, "posts_df.csv"))
writer_profile_df.to_csv('writer_profile_df.csv')
